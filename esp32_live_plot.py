...
# Features:
#  - EMA (Exponential Moving Average) with configurable time constant (--ema-tau)
#  - Rolling linear trend with slope [°C/min] and R²
#  - Median filter (5 samples) and simple outlier guard for data spikes
#
# Usage:
...
def serial_reader(port, baud, out_q, stop_event):
    if serial is None:
        print("pyserial missing. Install: pip install pyserial", file=sys.stderr)
        return
    try:
        ...
        print(f"Cannot open port {port} @ {baud}: {e}", file=sys.stderr)
    ...
    try:
        ...
        while not stop_event.is_set():
            try:
                ...
                print(f"[tcp] error: {e}", file=sys.stderr); time.sleep(0.2)
    except Exception as e:
        print(f"Cannot connect to {addr}: {e}", file=sys.stderr)
    finally:
        try: sock.close()
        except: pass

def ema_series(t_list, y_list, tau=30.0):
    """EMA with time constant tau [s]. Adaptive alpha = 1 - exp(-dt/tau) for irregular intervals."""
    if not t_list:
        return []
    y_ema = []
    prev = y_list[0]
...
def compute_trend(xs_time, ys, seconds_window):
    """Rolling linear regression over the last time window. Returns (trend_x, trend_y, slope_per_sec, r2)."""
    if len(xs_time) < 2:
        return [], [], None, None
    t_now = xs_time[-1]
...
def main():
    ...
    ap.add_argument("--port", help="Serial port, e.g. COM3/COM5 or /dev/ttyUSB0")
    ap.add_argument("--baud", type=int, default=115200, help="Baud rate (default 115200)")
    ap.add_argument("--interval", type=float, default=5.0, help="Plot refresh interval [s]")
    ap.add_argument("--window", type=float, default=300.0, help="Trend window [s]")
    ap.add_argument("--max-points", type=int, default=5000, help="Max samples in memory")
    ap.add_argument("--ema-tau", type=float, default=30.0, help="EMA time constant τ [s] (0 = disable EMA)")
    ap.add_argument("--list-ports", action="store_true", help="List available ports and exit")
    ap.add_argument("--tcp", help="TCP mode host:port (Wi‑Fi). If set, --port is ignored")
    args = ap.parse_args()
    ...
    if (args.port is None) and (args.tcp is None):
        print("Provide --port (USB) or --tcp host:port (Wi-Fi).", file=sys.stderr)
        sys.exit(2)
    ...
    if args.tcp:
        try:
            ...
            print("Invalid --tcp format. Use host:port, e.g. 192.168.4.1:3333", file=sys.stderr)
            sys.exit(2)
    ...
    ys = deque(maxlen=args.max_points)
    med_buf = deque(maxlen=5)
    lines_seen = 0
        ...
        while True:
            try:
                while True:
                    ...
                    # === OUTLIER GUARD: drop spikes >2°C within <2.5 s ===
                    if len(xs) >= 1:
            ...
            if (now - last_draw) >= args.interval and len(xs) >= 1:
                ...
                # Trend: slope and R² over args.window
